...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try {
        ...
        // Important: Merge parsed with defaults to ensure useBetaDeploy is handled
  ...
  const syncHistory = async (token: string, silent = false) => {
      ...
      try {
          ...
          if (!silent) addLog("Failed to sync history.", 'error');
      } finally {
  ...
  const saveHistory = async (newHistory: SavedProject[]) => {
    ...
    if (config.githubToken) {
        try {
            await saveHistoryToGist(config.githubToken, newHistory);
        } catch (e) {
  ...
  const saveCurrentProject = () => {
      ...
      if (!isDuplicate) {
        ...
        addLog(`Project "${project.name}" is already saved.`, 'warning');
      }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setConfig(newConfig);
      saveConfig(newConfig); // Persist immediately
      
      addLog(`Hello, ${username}! Login successful.`, 'success');
      setStep(Step.PROMPT);
      syncHistory(config.githubToken);
  ...
  const handleGenerate = async () => {
    ...
    if (mode === 'generate') {
        addLog(`I'm brainstorming code for: "${prompt.slice(0, 30)}..."`);
    } else {
        addLog('Analyzing code structure...');
    }
      ...
      if (config.useBetaDeploy && config.vercelToken) {
          ...
          try {
             ...
             if (vProject) {
              ...
              addLog(`[Beta] Auto-deploy failed (${e.message}). Falling back to manual mode.`, 'warning');
          }
      } else {
          addLog('3. Skipping auto-deploy (Beta disabled or no token).', 'info');
      }

      setDeploymentResult({
        repoUrl: repoData.html_url,
        deployUrl: vUrl ? vUrl : undefined,
        isBeta: isBetaSuccess
      });
             ...
             <div className="w-80 h-full bg-[#111] border-r border-deploy-border shadow-2xl animate-in slide-in-from-left duration-300 flex flex-col">
                ...
                {config.githubToken && (
                    <div className="p-3 border-b border-deploy-border bg-[#0a0a0a]">
                        <button 
                            onClick={() => syncHistory(config.githubToken)} 
                            disabled={isSyncing}
                            className="w-full text-xs flex items-center justify-center gap-2 bg-[#222] hover:bg-[#333] py-2 rounded text-blue-400 border border-blue-900/30 transition-all"
                        >
                            {isSyncing ? <Loader2 size={12} className="animate-spin"/> : <Cloud size={12}/>}
                            {isSyncing ? "Syncing..." : "Sync Cloud History"}
                        </button>
                    </div>
                )}
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {savedProjects.length === 0 ? (
                        <p className="text-gray-500 text-sm text-center italic mt-10">No saved projects yet.</p>
                    ) : (
                        savedProjects.map(p => (
        ...
        <div className="p-6 md:p-10 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-8">
                ...
                <div className="bg-deploy-card p-8 rounded-xl border border-deploy-border shadow-2xl relative overflow-hidden">
                    ...
                    <div className="space-y-5">
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full bg-black border border-deploy-border rounded-lg p-3 pl-10 text-sm focus:border-blue-500 focus:outline-none transition-colors"
                            ...
                            <div className="mt-2 flex justify-between text-xs">
                                ...
                                <a href="https://github.com/settings/tokens/new?scopes=repo,gist" target="_blank" rel="noreferrer" className="text-blue-400 hover:underline flex items-center gap-1">
                        ...
                        <div className="pt-4 border-t border-white/5">
                            ...
                            <input
                                type="password"
                                placeholder="AbCdEfGxxxxxxxx"
                                className="w-full bg-black border border-deploy-border rounded-lg p-3 text-sm focus:border-blue-500 focus:outline-none transition-colors"
                                value={config.vercelToken}
                                onChange={(e) => setConfig({ ...config, vercelToken: e.target.value })}
                            />
                            <a href="https://vercel.com/account/tokens" target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline mt-1 inline-block">
                                Get Vercel Token
                            </a>
                        </div>
                        ...
                        <div className={`mt-2 flex items-center gap-3 p-3 rounded-lg border transition-colors ${config.vercelToken ? 'bg-blue-900/10 border-blue-900/30' : 'bg-gray-900/30 border-gray-800 opacity-50'}`}>
                            <input 
                                type="checkbox" 
                                id="betaDeploy"
                                disabled={!config.vercelToken}
                                checked={config.useBetaDeploy || false}
                                onChange={(e) => setConfig({...config, useBetaDeploy: e.target.checked})}
                            ...
                            <label htmlFor="betaDeploy" className={`text-xs flex-1 ${config.vercelToken ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
                                <span className={`font-bold flex items-center gap-1 ${config.vercelToken ? 'text-blue-400' : 'text-gray-500'}`}>
                                    <Zap size={12} className={config.vercelToken ? "fill-blue-500 text-blue-500" : "text-gray-500"}/> 
                                    Enable Beta Auto-Deploy
                                </span>
                                <span className="block text-gray-500">I will create the Vercel project for you. (Requires Vercel Token)</span>
                            </label>
                        </div>

                        <button type="submit" className="w-full bg-white text-black font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition-all hover:bg-gray-200 mt-4">
                            Connect & Login <ArrowRight size={16} />
                        </button>
                    </div>
                </div>
            </form>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-500">
                ...
                <div className="bg-deploy-card p-6 rounded-xl border border-deploy-border shadow-2xl relative">
                    ...
                    <div className="relative">
                        <textarea
                            className="w-full bg-black border border-deploy-border rounded-md p-4 text-sm focus:border-blue-500 focus:outline-none min-h-[300px] resize-none font-mono leading-relaxed"
                            placeholder={mode === 'generate' ? "Example: I want a portfolio website..." : "// Paste your file contents here..."}
                            value={prompt}
